...
class ARVEHICLES_OT_create_ucx_collision(bpy.types.Operator):
    """Create UCX collision (physics) for the vehicle"""
    ...
    simplify_method: bpy.props.EnumProperty(
        ...
        description="Method to create the UCX collision",
        items=[
            ('CONVEX_HULL', "Convex Hull", "Create a simplified convex hull around the vehicle"),
            ('BOUNDS', "Bounding Box", "Create a simple box around the vehicle"),
            ('DECIMATE', "Decimated Mesh", "Create a simplified version of the vehicle mesh")
        ],
        default='CONVEX_HULL'
    )
    ...
    def execute(self, context):
        ...
        if self.simplify_method == 'BOUNDS':
            ...
            # Set layer_preset custom property
            joined_obj["layer_preset"] = "Collision_Vehicle"
            joined_obj["usage"] = "PhyCol"
...
class ARVEHICLES_OT_create_firegeo_collision(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Report success
        if len(collision_objects) == 1:
            self.report({'INFO'}, "Created FireGeo collision mesh")
        else:
